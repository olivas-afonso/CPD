...
void check_zeros (int N, long aux_x, long aux_y, long aux_z, long *search_x, long *search_y, long *search_z, long *aux_search_y, 
    long *aux_search_z){
...
int death_rule(int N, char *** grid, long aux_x, long aux_y, long aux_z)
{
    ...
    check_zeros (N, aux_x, aux_y, aux_z, &search_x, &search_y, &search_z, &aux_search_y, &aux_search_z);
    ...
    for(x=0; x < 3; x++, search_x++) 
    {
        for(search_y=aux_search_y, y=0; y < 3; y++, search_y++)
        {
            for(search_z=aux_search_z, z=0; z< 3;z++, search_z++)
            {
                ...
                ((search_x % N) != aux_x || (search_y % N) != aux_y || (search_z % N) != aux_z)){       
                    ++cont_rule;
