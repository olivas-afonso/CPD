...
int main(int argc, char *argv[]) {
    ...
    for( aux=0; aux < dims[2]; aux++)
    {
        ...
        My_MPI_Cart_Shift(cart_comm, 2, 1, 0, 0, dims[2]-1, 1, &source_rank, &diag_rank); // dir tras cima
        //My_MPI_Cart_Shift(cart_comm, 2, 1, 0, 1, 0, 1, &source_rank, &diag_rank); // dir cima
        MPI_Sendrecv(&data_send[0][0][aux], dims[2], MPI_INT, source_rank, 0, &data_recv_down[0][0][aux], dims[2], MPI_INT, diag_rank, 0, cart_comm, MPI_STATUS_IGNORE); // AR cima dir
