...
int main(int argc, char *argv[]) {
    ...
    for( aux_z=0; aux_z < TAMANHO_GRID; aux_z++)
    {
        ...
        MPI_Sendrecv(&data_send[aux_z][TAMANHO_GRID-1][0], dims[2], MPI_INT, esq_frente_rank, 0, &data_recv_dir[TAMANHO_GRID+1][aux_z], dims[2], MPI_INT, dir_tras_rank, 0, cart_comm, MPI_STATUS_IGNORE); // AR dir baixo

        //FACE ESQUERDA DIAGS
        MPI_Sendrecv(&data_send[aux_z][0][TAMANHO_GRID-1], dims[2], MPI_INT, dir_tras_rank, 0, &data_recv_esq[0][aux_z], dims[2], MPI_INT, esq_frente_rank, 0, cart_comm, MPI_STATUS_IGNORE); // AR esq baixo
        MPI_Sendrecv(&data_send[aux_z][TAMANHO_GRID-1][TAMANHO_GRID-1], dims[2], MPI_INT, dir_frente_rank, 0, &data_recv_esq[TAMANHO_GRID+1][aux_z], dims[2], MPI_INT, esq_tras_rank, 0, cart_comm, MPI_STATUS_IGNORE); // AR esq baixo
        ...
        for (aux_y=0; aux_y<TAMANHO_GRID; aux_y++)
        {
            ...
            //FACE ESQUERDA
            MPI_Sendrecv(&data_send[aux_y][aux_z][TAMANHO_GRID-1], dims[2], MPI_INT, dir_rank, 0, &data_recv_esq[aux_y+1][aux_z], dims[2], MPI_INT, esq_rank, 0, cart_comm, MPI_STATUS_IGNORE); // face dir
