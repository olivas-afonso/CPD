    ...
    for( aux_z=0; aux_z < TAMANHO_GRID; aux_z++)
    {
        for (aux_y=0; aux_y<TAMANHO_GRID; aux_y++)
        {
            ...
            for(aux_x=0;aux_x<TAMANHO_GRID; aux_x++)
            {
                ...
                MPI_Sendrecv(&data_send[0][aux_y][TAMANHO_GRID-1], dims[2], MPI_INT, dir_baixo_rank, 0, &data_recv_cima[TAMANHO_GRID+1][aux_x], dims[2], MPI_INT, esq_cima_rank, 0, cart_comm, MPI_STATUS_IGNORE); // AR esq baixo
            }
        }
